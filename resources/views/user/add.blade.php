...
{{-- @dd($user); --}}
        <form class="w-full max-w-lg" method="post" action={{isset($user)? route('user.update',$user->id):route('user.store') }}>
            ...
            <div class="px-3 -mx-3 mb-6">
                ...
                <input
                    ...
                    value={{old('content')?old('content'):(isset($user)?$user->content:"")}}>
            ...
            <div class="px-3 -mx-3 mb-6">
                ...
                <input
                    class="appearance-auto block w-full bg-gray-200 text-gray-700 border border-gray-200 rounded py-3 px-4 mb-3 leading-tight focus:outline-none bg-white focus:border-gray-500"
                    id="grid-first-name" type="file" placeholder="Img" name="img" value={{isset($user)?$user->img:""}}>
                    <p class="text-red-500 text-xs italic">{{$errors->get('img') ? $errors->get('img')[0]:''}}</p>
            </div>

            <button type="submit"
                class="bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded">{{isset($user)?"Update":"Add New"}}</button>
        </form>
